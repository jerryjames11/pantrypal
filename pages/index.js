...
function PriceDelta({ delta }) {
  ...
  const up = delta > 0
  return (
    <span className={up ? styles.priceUp : styles.priceDown} title={`${up ? '+' : ''}${fmt(delta)} vs last purchase`}>
...
function Spinner({ small }) {
  return <span className={small ? styles.spinnerSm : styles.spinner} aria-hidden="true" />
}
...
export default function PantryPal() {
  ...
  const [scanResult, setScanResult] = useState(null)   // { receipt, items } | { error }
  ...
  async function signOut() {
    ...
    setPantry([]); setReceipts([]); showToast('Signed out')
  }
  ...
  function clearImage() {
    setPreviewSrc(null); setImgBase64(null); setImgMime(null)
  ...
  // ── Render ───────────────────────────────────────────────────────────────
  if (authLoading) return <div className={styles.centered}><Spinner /></div>
    ...
    <div className={styles.app}>
      ...
      <nav className={styles.tabs}>
        {[['pantry','📋','Pantry'],['scan','📷','Scan'],['history','🧾','History'],['recipes','🍳','Recipes']].map(([id, icon, label]) => (
          <button key={id} className={`${styles.tabBtn} ${tab === id ? styles.activeTab : ''}`} onClick={() => setTab(id)}>
        ...
        <section>
          ...
          <div className={styles.filterRow}>
            {['all','fresh','low','out'].map(f => (
              <button key={f} className={`${styles.chip} ${pantryFilter === f ? styles.chipOn : ''}`}
          ...
          {pantryLoading ? (
            <div className={styles.loadRow}><Spinner small /> Loading pantry…</div>
          ) : filtered.length === 0 ? (
            <div className={styles.empty}>{stats.total === 0 ? 'No items yet — scan a receipt or add items above' : 'Nothing in this category'}</div>
          ) : (
              ...
              {filtered.map(item => (
                <div key={item.id} className={styles.itemCard}>
                  ...
                  <div className={styles.itemRow2}>
                    {item.qty ? <span className={styles.itemQty}>{item.qty}</span> : <span />}
                    {item.last_price != null && <span className={styles.itemPrice}>{fmt(item.last_price)}</span>}
                  </div>
                  <div className={styles.itemFoot}>
                    <select className={`${styles.statusSel} ${styles['s_' + item.status]}`}
                      value={item.status} onChange={e => updateStatus(item.id, e.target.value)}>
                      <option value="fresh">✓ In stock</option>
                      <option value="low">↓ Low</option>
                      <option value="out">✕ Out</option>
                    </select>
                    <button className={styles.iconBtn} onClick={() => removeItem(item.id, item.name)} aria-label={`Remove ${item.name}`}>✕</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {tab === 'scan' && (
        <section>
          {!user && <div className={styles.warnBanner}>Sign in above to save scanned receipts to your account.</div>}
          ...
          {!previewSrc ? (
            ...
            <div className={styles.previewWrap}>
              ...
              <button className={styles.previewClear} onClick={clearImage}>✕ Clear</button>
            </div>
          )}
          ...
          <textarea className={styles.textarea} value={receiptText}
            onChange={e => setReceiptText(e.target.value)} rows={5}
            placeholder="Paste receipt text, a grocery list, or just type items and prices…" />
          <button className={`${styles.btn} ${styles.btnPrimary} ${styles.btnWide}`} style={{ marginTop: 8 }}
            disabled={scanLoading || !receiptText.trim()}
            onClick={() => doScan({ text: receiptText })}>
            {scanLoading ? <><Spinner small /> Parsing…</> : '✓ Parse text with AI'}
          </button>
      ...
      {tab === 'history' && (
        <section>
          {!user ? (
            ...
            <div className={styles.loadRow}><Spinner small /> Loading receipts…</div>
          ) : receipts.length === 0 ? (
              ...
              <div key={r.id} className={styles.receiptCard}>
                <div className={styles.receiptHeader} onClick={() => setExpandedReceipt(expandedReceipt === r.id ? null : r.id)}>
                  ...
                  <div className={styles.receiptActions}>
                    <button className={styles.iconBtn} onClick={e => { e.stopPropagation(); deleteReceipt(r.id) }} aria-label="Delete receipt">🗑</button>
                  ...
                  <div className={styles.receiptItems}>
                    ...
                    {(r.receipt_items || []).map(item => (
                      <div key={item.id} className={styles.receiptItemRow}>
                        ...
                        <span className={styles.riDelta}><PriceDelta delta={item.price_delta} /></span>
                      </div>
                    ))}
      ...
      {tab === 'recipes' && (
        <section>
          <button className={`${styles.btn} ${styles.btnPrimary} ${styles.btnWide}`}
            style={{ marginBottom: 16 }} onClick={getRecipes} disabled={recipeLoading}>
            {recipeLoading ? <><Spinner small /> Finding matches…</> : '✨ Suggest recipes from my pantry'}
          </button>

          {pantry.filter(i => i.status !== 'out').length < 2 && !recipeLoading && recipes.length === 0 && (
            <div className={styles.empty}>Add at least a couple items to your pantry first.</div>
          )}
            ...
            <div key={i} className={styles.recipeCard}>
              ...
              {openSteps[i] && (
                <div className={styles.stepsBody}>
                  {r.steps?.map((s, n) => (
                    <div key={n} className={styles.step}><span className={styles.stepN}>{n + 1}.</span><span>{s}</span></div>
                  ))}
                </div>
              )}
            </div>
