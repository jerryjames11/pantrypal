...
export default function PantryPal() {
  ...
  async function addRecipeIngredientsToCart(recipe) {
    ...
    showToast(`${needed.length} ingredients added to cart`); loadCart()
    setTab('cart')
  }
  ...
  if (authLoading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh' }}><Spinner /></div>
    ...
    <div className={styles.app}>
      ...
      <header className={styles.header}>
        <div className={styles.brand}>
          ...
          <div><div className={styles.appTitle}>PantryPal</div><div className={styles.appSub}>Track groceries · discover recipes</div></div>
        </div>
        <div>
          {user ? (
      ...
      {!user && <div className={styles.guestBanner}><strong>Sign in</strong> to save your pantry and access it from any device.</div>}

      {/* TABS */}
      <nav className={styles.tabs}>
        {[['pantry', '📋', 'Pantry'], ['scan', '📷', 'Scan'], ['history', '🧾', 'History'], ['recipes', '🍳', 'Recipes'], ['cart', '🛒', 'Cart']].map(([id, icon, label]) => (
          <button key={id} className={tab === id ? `${styles.tabBtn} ${styles.activeTab}` : styles.tabBtn} onClick={() => setTab(id)}>
            {icon} {label}{id === 'cart' && cart.length > 0 && <span className={styles.cartBadge}>{cart.length}</span>}
          </button>
        ))}
      </nav>
      ...
      {tab === 'scan' && (
        <section>
          {!user && <div className={styles.warnBanner}>Sign in to save scanned receipts to your account.</div>}
      ...
      {tab === 'history' && (
        <section>
          {!user ? <div className={styles.empty}>Sign in to view your receipt history.</div>
        ...
        <section>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <button className={styles.scanBtn} style={{ flex: 1 }} onClick={getRecipes} disabled={recipeLoading}>
              {recipeLoading ? 'Finding matches…' : '✨ Suggest recipes'}
            </button>
                  ...
                  <div key={r.id} className={styles.recipeCard}>
                    <div className={styles.recipeTop}>
                      ...
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <button onClick={() => addRecipeIngredientsToCart(r)} className={styles.chip} title="Add missing ingredients to cart">🛒</button>
                        <button onClick={() => unsaveRecipe(r.id)} className={styles.chip} title="Remove saved recipe">🗑</button>
                      </div>
                    </div>
              ...
              <div key={i} className={styles.recipeCard}>
                <div className={styles.recipeTop}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    <button onClick={() => isSaved ? null : saveRecipe(r)} title={isSaved ? 'Saved!' : 'Save recipe'}
                      style={{ background: 'none', border: 'none', cursor: isSaved ? 'default' : 'pointer', fontSize: 18 }}>
                      {isSaved ? '❤️' : '🤍'}
                    </button>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center' }}>
                  ...
                  {r.need?.length > 0 && (
                    <button className={styles.chip} onClick={() => addRecipeIngredientsToCart(r)} title="Add missing ingredients to cart">🛒 Add missing to cart</button>
                  )}
                </div>
                {openSteps[i] && (
      ...
      {tab === 'cart' && (
        <section>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            {cartCheckedCount > 0 && (
              <button className={styles.chip} onClick={clearCheckedCart}>🗑 Remove checked ({cartCheckedCount})</button>
            )}
          </div>
          ...
          {cartLoading ? (
            <div className={styles.loadRow}><Spinner /> Loading cart…</div>
          ) : cart.length === 0 ? (
            <div className={styles.empty}>Cart is empty — add items above or pull from pantry</div>
          ) : (
