...
export async function estimateExpiryDate(itemName, fromDate) {
  const baseDate = fromDate ? new Date(fromDate) : new Date()
  const heuristicDays = matchExpiryDays(itemName)
  if (heuristicDays !== null) {
    console.log(`estimateExpiryDate: "${itemName}" -> heuristic match -> ${heuristicDays} days from ${baseDate.toISOString().slice(0,10)}`)
    return calcExpiryDate(heuristicDays, baseDate)
  }
  ...
  return calcExpiryDate(aiDays, baseDate)
}
